...
XML_PATH = os.environ.get("SPIDERBOT_XML_PATH", str(SCRIPT_DIR / "xmls" / "stairs.xml"))
# Override with a policy exported for this script's observation/action configuration.
ONNX_MODEL_PATH = os.environ.get(
  "SPIDERBOT_POLICY_PATH", str(SCRIPT_DIR / "policies" / "rough.onnx")
)
    ...
    while gui.is_running():
      ...
      if current_time - last_control_time >= dt_control:
        ...
        if step_count == TEST_FORWARD_AFTER_STEPS:
          ...
          print(f"{'=' * 60}\n")
          cmd_interface.lin_vel_x = 0.15  # Move forward
          # cmd_interface.ang_vel_z = 0.5  # Uncomment to test turning

...
def main():
    ...
    print(f"Phase 2: Walking at {COMMAND_VEL_X} m/s for {LOCOMOTION_STEPS} steps ({LOCOMOTION_STEPS/CONTROL_FREQ:.1f} s)")
        ...
        while gui.is_running() and step_count < total_steps:
            ...
            if current_time - last_control_time >= dt_control:
                ...
                if step_count == STILL_STEPS:
                    ...
                    print(f"[Step {step_count}] Locomotion phase started. "
                ...
                if step_count % 50 == 0:
                    ...
                    print(f"[{phase}] Step {step_count:4d} | "
    ...
    # ── CoT Report ────────────────────────────────────────────────────────
    loco_end_pos    = data.qpos[:2].copy()
    displacement    = np.linalg.norm(loco_end_pos - loco_start_pos)  # straight-line, for reference
    g = 9.81
    ...
    cot = loco_energy_J / (ROBOT_MASS_KG * g * loco_distance_m) if loco_distance_m > 0.01 else float('nan')

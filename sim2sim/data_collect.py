...
OUTPUT_DIR = os.environ.get(
  "SPIDERBOT_OUTPUT_DIR", str(SCRIPT_DIR.parent / "logs" / "locomotion_data")
)
...
class DataTracker:
  ...
  def save_data(self):
    ...
    if len(self.last_link_positions) > 0:
      ...
      print(
        f"   Overall mean position: [{overall_mean[0]:.4f}, {overall_mean[1]:.4f}, {overall_mean[2]:.4f}]"
      )
      print(
        f"   Overall std deviation: [{overall_std[0]:.4f}, {overall_std[1]:.4f}, {overall_std[2]:.4f}]"
      )
  ...
  def print_summary(self):
    ...
    if len(self.last_link_positions) > 0:
      ...
      print(
        f"  Mean position: [{overall_mean[0]:.4f}, {overall_mean[1]:.4f}, {overall_mean[2]:.4f}] m"
      )
      print(
        f"  Std deviation: [{overall_std[0]:.4f}, {overall_std[1]:.4f}, {overall_std[2]:.4f}] m"
      )
...
def parse_args():
  parser = argparse.ArgumentParser(
    description="Collect Spiderbot sim2sim rollout data."
  )
  parser.add_argument(
    "--history-length",
    type=int,

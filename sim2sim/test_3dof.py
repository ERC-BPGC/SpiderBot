...
XML_PATH = os.environ.get(
  "SPIDERBOT_XML_PATH", str(SCRIPT_DIR / "xmls" / "flat_3dof.xml")
)
# Override with a policy exported for this script's observation/action configuration.
ONNX_MODEL_PATH = os.environ.get(
  "SPIDERBOT_POLICY_PATH", str(SCRIPT_DIR / "policies" / "flat_3dof.onnx")
)
...
def parse_args():
  parser = argparse.ArgumentParser(
    description="Run 3-DOF Spiderbot sim2sim policy playback."
  )
  parser.add_argument(
    "--history-length",
    type=int,
...
def main():
  ...
  for i, (joint_name, act_idx) in enumerate(
    zip(ACTUATED_JOINT_NAMES, actuator_indices, strict=True)
  ):
    ...
    while gui.is_running():
      ...
      if current_time - last_control_time >= dt_control:
        ...
        if step_count == TEST_FORWARD_AFTER_STEPS:
          ...
          print(f"{'=' * 60}\n")
          cmd_interface.lin_vel_x = 0.25  # Move forward
          cmd_interface.ang_vel_z = 0.0  # Uncomment to test turning
        ...
        if step_count % 50 == 0:  # Print every second at 50Hz
          ...
          print(
            f"  Base ang vel: [{base_ang_vel[0]:+.3f}, {base_ang_vel[1]:+.3f}, {base_ang_vel[2]:+.3f}]"
          )
          print(
            f"  Action mean: {action.mean():.3f}, std: {action.std():.3f}, range: [{action.min():.3f}, {action.max():.3f}]"
          )
          print(
            f"  Base pos (xyz): [{data.qpos[0]:.3f}, {data.qpos[1]:.3f}, {data.qpos[2]:.3f}]"
          )
          print()

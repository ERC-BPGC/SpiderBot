    ...
    while gui.is_running():
      ...
      if current_time - last_control_time >= dt_control:
        ...
        if step_count % 50 == 0:  # Print every second at 50Hz
          ...
          print(
            f"  Raw action mean: {last_action.mean():.3f}, std: {last_action.std():.3f}, range: [{last_action.min():.3f}, {last_action.max():.3f}]"
          )
          print(
            f"  Joint delta mean: {action.mean():.3f}, std: {action.std():.3f}, range: [{action.min():.3f}, {action.max():.3f}]"
          )
          print(
